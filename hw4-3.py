...
def display_directory_structure(directory_path, indent=0):
    try:
        ...
        if not path.is_dir():
            raise ValueError("Вказаний шлях не веде до директорії.")

        # Виведення імен директорій та файлів з використанням кольорів
...
'''  Переконайтеся, що ви встановили бібліотеку colorama перед виконанням скрипта:
GitBash
pip install colorama
Цей скрипт виводить структуру директорії з використанням синього кольору для директорій та зеленого кольору для файлів.
Якщо вказаний шлях не існує або не є директорією, виводиться відповідне повідомлення про помилку.
'''
